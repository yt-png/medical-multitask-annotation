...
> - **Sprint C（进行中）**：M4.3 返工预填源仅 `previous_annotations`（已完成）；M6.4 快照语义文档收口（已完成）；M8.1 三任务合并 + final 自包含核实（已完成）；M8.2 merge 无必须有 prelabel（已完成）；M8.3 merge 回归（已完成）；M12.4 SEG/DET/CAP 独立运行（已完成）；仍待 M12.5。
> - **仍未完成（Sprint C 余项 / D）**：场景门禁 M12.5；可选 B2（返工路径脱离 PrelabelItem）；`deploy/v1`（M11）；文档冻结（M0）；M12.6。详见 CHANGELOG「Planned」与 `.cursor/rules/V1 Development Tasks.md`。
